...
# FIND ALL EVENTS
cursor = coll.aggregate([ {'$match': {'Actor1Code': 'CHN', 'Actor2CountryCode': { '$nin': [ 'CHN'] }}}, { '$sample': { 'size': 1000000 }}], allowDiskUse= True )
data = list(cursor)
cursor.close()
...
# df.groupby(['MonthYear','Actor2CountryCode'])['_id'].nunique().nlargest(30).unstack('Actor2CountryCode').plot()
