...
# FIND EVENTS
events = coll.aggregate([{'$match': {'ActionGeo_CountryCode': {'$in': [
                        'BR', 'CH', 'IN', 'IT', 'PL', 'RS', 'SP', 'TU', 'UK', 'US']}}}], allowDiskUse=True)
data = list(events)
events.close()
...
df.groupby([pd.Grouper(key='SQLDATE', freq='W'), 'ActionGeo_CountryCode'])[
    '_id'].nunique().unstack('ActionGeo_CountryCode').plot()
...
print(x, y)
plot = df.groupby('ActionGeo_CountryCode')['GoldsteinScale'].mean(
).sort_values().plot(kind='bar', color='paleturquoise')
...
# ACTOR 1
events = coll.aggregate([{'$match': {'Actor2Geo_CountryCode': {'$in': [
                        'BR', 'CH', 'IN', 'IT', 'PL', 'RS', 'SP', 'TU', 'UK', 'US']}}}], allowDiskUse=True)
data = list(events)
events.close()
df = pd.DataFrame(data)
